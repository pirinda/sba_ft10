...
public class DViewTestAppResult extends DGridPaneView {
    ...
    public void prepareSqlQuery() {
        ...
        msSql = "SELECT " +
                "arv.id_job AS " + DDbConsts.FIELD_ID + "1, " +
                "arv.id_app AS " + DDbConsts.FIELD_ID + "2, " +
                "arv.id_res AS " + DDbConsts.FIELD_ID + "3, " +
                "arv.id_var AS " + DDbConsts.FIELD_ID + "4, " +
                "arv.val, " +
                "j.num, " +
                "j.dat, " +
                ...
                "arv.id_job = ar.id_job AND arv.id_app = ar.id_app AND arv.id_res = ar.id_res " +
                "INNER JOIN " + DModConsts.TablesMap.get(DModConsts.Q_APP) + " AS a ON " +
                "ar.id_job = a.id_job AND ar.id_app = a.id_app " +
                ...
                "ORDER BY j.num, t.name, t.code, i.name, i.code, arv.id_job, arv.id_app, arv.id_res, v.name, v.code, arv.id_var ";
    }
    ...
    public void createGridColumns() {
        ...
        columns[col++] = new DGridColumnView(DGridConsts.COL_TYPE_INT_1B, DDbConsts.FIELD_ID + "3", "# resultado");
        columns[col++] = new DGridColumnView(DGridConsts.COL_TYPE_TEXT_NAME_CAT_S, "v.name", DGridConsts.COL_TITLE_NAME + " variable");
        columns[col++] = new DGridColumnView(DGridConsts.COL_TYPE_TEXT_CODE_CAT, "v.code", DGridConsts.COL_TITLE_CODE + " variable");
        columns[col++] = new DGridColumnView(DGridConsts.COL_TYPE_DEC_AMT_UNIT, "arv.val", "Valor");
        columns[col++] = new DGridColumnView(DGridConsts.COL_TYPE_TEXT_CODE_CAT, "v.uom", "Unidad");
